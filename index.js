...
function getFinals(Arr) {
    const Finalists = Arr.filter((item) => {
...
function getWinners(Arr, cb) {
    return cb(Arr).map(item => item['Home Team Goals'] > item['Away Team Goals'] ? item['Home Team Name'] : item['Away Team Name']);
};
...
function getWinnersByYear(Arr, finalsCB, yearsCB, winnersCB) {
    ...
    return winner.map((item, index) => `In ${year[index]}, ${winner[index]} won the world cup!` )
}
...
function getAverageGoals(cb) {
   const averageHomeGoals = cb.reduce(function(acc, item){
